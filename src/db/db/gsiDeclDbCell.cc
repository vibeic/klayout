...
Class<db::Cell> decl_Cell ("db", "Cell",
  ...
  gsi::method_ext ("fill_region", &fill_region, gsi::arg ("region"),
    ...
    "\n"
    "With local origin optimization ('origin' is nil), each polygon of the region is filled with its own fill pattern origin, "
    "so two neighboring polygons produce two pitch-incompatible arrays. The 'fill_margin' parameter therefore also acts between "
    "the polygons: a fill cell is not placed within the margin of a fill cell already placed for another polygon of the same "
    "region. This is what gives the safety distance between such arrays. With a global origin all polygons share one raster and "
    "no such distance applies.\n"
    "\n"
